...
void options (int argc, char **argv) {
  ...
  while (i < argc) { DerDevice *d;
	...
	|| uri_retrieval (argv[i])) {
      ...
      client_init (name, argv[i]); secure = 1; // now it's secure after init client
  ...
  while (i < argc) { uint64_t sfdi;
    // printf("IN client_test.c: options()\n"); // ADDED!!!  haven't reached this point yet
...
int main (int argc, char **argv) {
  ...
  version ();
  platform_init ();
  printf("IN client_test.c: main()\n"); // ADDED!!!
  options (argc, argv);
  printf("...main(): after options()\n"); // ADDED!!!
  while (1) {
    switch (der_poll (&any, -1)) {
    case SERVICE_FOUND: s = any;
      printf("...main(): SERVICE_FOUND\n"); // ADDED!!!
      print_service (s);
      if (test) {
        printf("...main(): in test\n"); //ADDED!!!
        get_dcap (s, secure);
    ...
    case TCP_ACCEPT:
      printf("...main(): TCP_ACCEPT\n"); // ADDED!!!
      // accept_notifier (any);
    case TCP_PORT:
      printf("...main(): TCP_PORT\n"); // ADDED!!!
      if (conn_session (any)) {
        http_debug (any, 1);
        // if (http_client (any))
      ...
      break;
    case TCP_TIMEOUT:
      printf("...main(): TCP_TIMEOUT\n"); // ADDED!!!
      cleanup_http (any);
      printf ("Connection timed out\n"); return 0;
    case TCP_CLOSED:
      printf("...main(): TCP_CLOSED\n"); // ADDED!!!
      cleanup_http (any);
