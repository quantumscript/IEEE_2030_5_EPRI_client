...
int tls_session (void *conn) {
  printf("IN connection.c: tls_session()\n"); // ADDED!!!
  Connection *c = conn;
  if (net_status (c) == Connected) {
    switch (c->tls_state) {
    case TLS_NEGOTIATE:
    if (ssl_handshake (c->tls)) {
      printf("***ssl_handshake\n"); // ADDED!!!
...
void tls_setup (Connection *c) {
  printf("IN connection.c: tls_setup()\n"); // ADDED!!!
...
void *conn_accept (void *conn, Acceptor *a, int secure) {
  printf("IN connection.c: conn_accept()\n"); // ADDED!!!
...
void *conn_connect (void *conn, Address *server, int secure) {
  printf("IN connection.c: conn_connect()\n"); // ADDED!!!
