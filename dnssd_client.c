...
char *txt_pair (char *key, char **value, char *data) {
  // printf("IN dnssd_client.c: txt_pair()\n"); // ADDED!!!
...
char *txt_value (char *buffer, char *data, char *key) {
  // printf("IN dnssd_client.c: txt_value\n"); // ADDED!!!
  if (data) { char *value;
...
void *dns_srv (Service *s, char *data) {
  // printf("IN dnssd_client.c: dns_srv()\n"); // ADDED!!!
...
void dns_ptr (char *name, char *data) {
  // printf("IN dnssd_client.c: dns_ptr()\n"); // ADDED!!!
  Question *q; Service *s; char instance[256]; int length;
  if ((q = find_question (name))
      && (data = dns_name (instance, data+10))) {
    // printf("...instance: %s\n", instance); // ADDED!!! 

    s = get_service (instance); s->query = q->name; // search for service based on domain name from PTR record
    // printf("...after get_service(instance)\n"); // ADDED!!!
...
void process_answers (char *data, int count) {
  // printf("IN dnssd_client.c: process_answers()\n"); // ADDED!!!  LEFT OFF HERE!!!
  ...
  while (count--) {
    if (data = parse_rr (name, &length, data)) {
      ...
      data = rr_next (data, length);
    }  //  FOUND BUG: return was here... shortcircutiing the while loop
  } return;
}

// process a DNS-SD packet
void dnssd_packet (char *data, int length) { DnsHeader header;
  // printf("IN dnssd_packet, dnssd_client.c\n"); // ADDED!!!
...
void dnssd_followup (UdpPort *p) {
  // printf("IN dnssd_client.c: dnssd_followup()\n");  // ADDED!!!
  ...
  while (s) { int complete = 1;
    ...
    if (!s->host->addr.length) {  // GIVING A SEGFAULT ERROR
...
// receive and process DNS-SD pakets, return list of services
Service *dnssd_receive (UdpPort *port) {
  // printf("IN dnssd_clinet.c: dnssd_receive()\n"); // ADDED!!!
...
Service *service_next (Service *s) {
  // printf("IN dnssd_client.c: service_next\n"); // ADDED!!!
  while (s) { // process discovered services
    // printf("...name: %s, port: %i, txt_found: %i\n", s->name, s->port, s->txt_found); // ADDED!!!
