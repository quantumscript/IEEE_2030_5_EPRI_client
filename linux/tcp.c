...
void net_connect (void *port, Address *server) {
  printf("IN tcp.c: net_connect()\n"); // ADDED!!!
  TcpPort *p = port;
  p->pe.socket = bsd_socket (server->family);
  event_add (p->pe.socket, p);
  p->pe.type = TCP_CONNECT;
  int ret_val = connect (p->pe.socket, (struct sockaddr *)server,
	       server->length); // ADDED!!! - refactoring
  perror("ERROR"); // ADDED!!!  < LEFT OFF HERE!!!!!!!!!
  if (ret_val == 0) { // CHANGED - refactoring
