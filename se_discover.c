...
void se_discover (int server, int qu) { int i = 0;
  // printf("IN se_discover.c: se_discover()\n"); // ADDED!!!
  ...
  while (server) {
    if (server & 1) { char *n = name;
      ...
      // strcpy (n, "._smartenergy._tcp.site");   // service_name[14] = "smartenergy"
      strcpy (n, "._smartenergy._tcp.local");   // ADDED!!! trying to match the service record on the server
      write_counted (name);
      // printf("...name: %s\n", name); // ADDED!!!
...
void discover_device () {
  // printf("IN se_discover.c: discover_device()\n"); // ADDED!!!
...
void se_discover_unicast (char *domain) {
  // printf("IN se_discover_unicast, se_discover.c\n"); // ADDED!!!
...
char *service_dcap (Service *s) {
  // printf("IN se_discover.c: service_dcap()\n");  // ADDED!!!
  static char buffer[64];
  return txt_value (buffer, s->txt, "dcap"); // RETURN VALUE IS THE HREF FOR get_resource in retrieve.c
}
...
void *service_connect (Service *service, int secure) {
  printf("IN se_discover.c: service_connect()\n"); // ADDED!!!
  Address *addr; char buffer[32]; char *https = NULL;
  if (secure) https = txt_value (buffer, service->txt, "https"); // https is a key in TXT record
  
  // int port = https? (*https == '\0'? 443 : atol (https)) : service->port,
  //   n_port = htons (port); // COMMENTED - this is very confusing to understand 
  *https == '\0'? 443 : atol (https);  // ADDED!!! - refactoring
  int port = atol(https); // ADDED!!! - refactoring
  
  printf ("...service_connect: connect on port %d, https = %s, port = %d\n", port, https, service->port); 
  addr = &service->host->addr; // addr->port = n_port; // COMMENTED - refactoring
  set_port(addr, port); // ADDED!!! - refactoring
  return se_connect (addr, https != NULL);
}

Service *service_receive (UdpPort *port) {
  // printf("IN se_discover.c: service_receive()\n"); // ADDED!!!
  if (port == mdns_port || port == mdns_source || port == dns_port) {
    // if (port == mdns_port) printf("...port is mdns_port\n"); // ADDED!!!
    // if (port == mdns_source) printf("...port is mdns_source\n"); // ADDED!!!
    // if (port == dns_port) printf("...port is dns_port\n"); // ADDED!!!
